...
var Renderer = /** @class */ (function () {
    ...
    Renderer.prototype.positionNodes = function () {
        ...
        var rows = Object.keys(nodesByRow), xValues = fp_1.map(function (node) {
            return node.x;
        })(this.layout.nodes), maxX = Math.max.apply(Math, xValues), config = this.state.current.get("config"), xGridSpacing = config.width / (maxX + 1), yGridSpacing = config.height / (rows.length + 1);
        // Assign y values
        fp_1.forEach(function (node) {
            node.y = (node.y + 1) * yGridSpacing;
        })(this.layout.nodes);
        fp_1.forEach(function (row) {
            fp_1.flow(fp_1.sortBy(function (node) {
                return node.x;
            }), fp_1.forEach(function (node) {
                node.x *= xGridSpacing;
