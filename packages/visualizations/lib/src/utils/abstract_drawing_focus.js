...
var AbstractDrawingFocus = /** @class */ (function (_super) {
    ...
    function AbstractDrawingFocus(state, stateWriter, events, el) {
        ...
        _this.events.on(event_catalog_1.default.FOCUS.ELEMENT.HOVER, _this.onElementHover(_this));
        _this.events.on(event_catalog_1.default.FOCUS.ELEMENT.OUT, _this.onElementOut(_this));
        _this.events.on(event_catalog_1.default.CHART.OUT, _this.onMouseLeave(_this));
        return _this;
    }
    AbstractDrawingFocus.prototype.onElementOut = function (ctx) {
        return function () {
            ctx.remove();
        };
    };
    AbstractDrawingFocus.prototype.onMouseLeave = function (ctx) {
        return function () {
            ctx.remove();
        };
    };
